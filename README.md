...
| Windows | `.msi` | 管理者向け（サイレントインストール対応） |

> **macOS サポートは一時休止中です。** 現在は Windows のみリリースしています。安定化作業は [`platform/macos`](https://github.com/SakuraRinDev/WhisperDrop/tree/platform/macos) ブランチで継続中で、準備ができ次第リリースを再開します。

## 使い方

1. インストール後、初回起動で Whisper モデルが自動ダウンロードされます（約 1.5GB）
2. `Ctrl+Shift+Space`（macOS: `Cmd+Shift+Space`）で録音開始
...
| キー | 動作 |
|---|---|
| `Ctrl+Shift+Space`（macOS: `Cmd+Shift+Space`） | 録音開始 / 停止 |
| 同上 x2（素早く） | ロックモード（長時間録音） |
| `Escape` | 録音・文字起こしをキャンセル |

### 括弧で囲む（録音中にタップ）

録音中に以下のキーをポンと押すと、転写結果を自動で括弧で囲みます（もう一度押すと解除）。
...
リリースはメンテナーが `v*` 形式の Git タグを push すると、GitHub Actions ([release.yml](.github/workflows/release.yml)) が自動でビルドして GitHub Releases に成果物を添付します。コントリビュータが手動でビルド・アップロードする必要はありません。

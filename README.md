...
| macOS | `.dmg` | 開いてアプリケーションフォルダにドラッグ（[初回起動時の注意](#macos-で壊れていますと表示される場合)） |
...
### macOS で「壊れています」と表示される場合

WhisperDrop の Mac 版は現在 Apple の公証 (notarization) に未対応のため、初回起動時に macOS の Gatekeeper が「"WhisperDrop" は壊れているため開けません。ゴミ箱に入れる必要があります」と表示することがあります。**実際にファイルが壊れているわけではありません。** 以下のコマンドで隔離属性を外してから起動してください。
